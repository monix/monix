...
object delayFirst {
  ...
  def onEvent[T](source: Observable[T], policy: BufferPolicy)
      (eventInit: (() => Unit, Throwable => Unit) => Cancelable): Observable[T] = {
    ...
    Observable.create[T] { subscriber =>
      ...
      source.unsafeSubscribe(new Observer[T] {
        ...
        private[this] def connect(): Unit = lock.synchronized {
          if (!isConnected) {
        ...
        def onComplete(): Unit = {
          ...
          connectedF.onContinueSignalComplete(observer)
        }

        def onError(ex: Throwable): Unit = lock.synchronized {
          if (!isConnected) {
            ...
            connectedF.onContinueSignalError(observer, ex)
          }
        }
      })
    }
   ...
   */
  def onFuture[T](source: Observable[T], policy: BufferPolicy, future: Future[_]): Observable[T] =
  ...
  def byTimespan[T](source: Observable[T], timespan: FiniteDuration, policy: BufferPolicy): Observable[T] = {
    Observable.create { subscriber =>
      implicit val s = subscriber.scheduler
      val underlying = source.delayFirstOnEvent(policy = policy, eventInit = {
