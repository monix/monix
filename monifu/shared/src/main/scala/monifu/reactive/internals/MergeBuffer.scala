...
import monifu.reactive.observers.{BufferedSubscriber, SynchronousBufferedSubscriber, SynchronousObserver}
...
private[monifu] final class BoundedMergeBuffer[U]
  ...
  private[this] val buffer = BufferedSubscriber(downstream, bufferPolicy)
    ...
    lock.synchronized {
      if (!isDone) {
        ...
        if (signalError ne null)
          buffer.observer.onError(signalError)
      }
    }

  def onNext(elem: U) = {
    buffer.observer.onNext(elem).onCancel(cancelStreaming())
  }
...
private[monifu] final class UnboundedMergeBuffer[U]
  (downstream: Observer[U], bufferPolicy: BufferPolicy)
      (implicit s: Scheduler) extends SynchronousObserver[U] { self =>

  private[this] val activeStreams = Atomic(1)
  private[this] val buffer = bufferPolicy match {
    case Unbounded =>
      SynchronousBufferedSubscriber.unbounded(downstream)
    case OverflowTriggering(bufferSize) =>
      SynchronousBufferedSubscriber.overflowTriggering(downstream, bufferSize)
    case _ =>
  ...
  def onNext(elem: U) =
    buffer.observer.onNext(elem) match {
