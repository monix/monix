...
object FromIterableSuite extends TestSuite[TestScheduler] {
  ...
  test("fromIterable should stream synchronously after loop was initiated") { implicit s =>
    ...
    Observable.fromIterable(Seq(1,2,3,4,5)).unsafeSubscribe(
      new Observer[Int] {
        def onNext(elem: Int) = {
          sum += elem
          Continue
  ...
  test("fromIterable should do empty iterables synchronously") { implicit s =>
    ...
    Observable.fromIterable(Seq.empty).unsafeSubscribe(
      new Observer[Int] {
        def onComplete(): Unit = wasCompleted = true
  ...
  test("fromIterable should protect against broken iterable.next, synchronous version") { implicit s =>
    ...
    Observable.fromIterable(iterable).unsafeSubscribe(
      new Observer[Int] {
        def onNext(elem: Int) = {
          sum += elem
          Continue
