...
//        val completed = new CountDownLatch(1)
//
//        val obs = Observable.range(1, 100)
//
//        var sum = 0
//        obs.subscribe(new Subscriber[Int] {
//          private[this] var s: Subscription = null
...
//    }
    ...
    it("must allow synchronous request calls from onNext and onSubscribe") {
      val completed = new CountDownLatch(1)
      var sum = 0

      val obs = Observable.repeat(1)
        .map(x => Observable.unit(x))
        .merge(bufferPolicy = BackPressured(20), batchSize = 2)
        .take(6)
        .publisher
      ...
      obs.subscribe(new Subscriber[Int] {
        ...
        def onError(t: Throwable): Unit = {
          globalScheduler.reportFailure(t)
        }
      ...
      assert(completed.await(5, TimeUnit.SECONDS),
        "completed.await should succeed")
      assert(sum === 6)
    }
  }
}
