...
package monifu.tckTests

import monifu.concurrent.Implicits.globalScheduler
import monifu.Ack.Continue
import monifu.Observer
import monifu.observers.SynchronousObserver
import monifu.tckTests.SubscriberWhiteBoxSyncTest.Value
...
class SubscriberWhiteBoxSyncTest
  ...
  def createSubscriber(probe: WhiteboxSubscriberProbe[Value]): Subscriber[Value] = {
    val underlying = Observer.toReactiveSubscriber(new SynchronousObserver[Value] {
