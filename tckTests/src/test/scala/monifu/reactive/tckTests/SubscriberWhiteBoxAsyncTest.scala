...
import monifu.reactive.tckTests.SubscriberWhiteBoxAsyncTest.Value
...
class SubscriberWhiteBoxAsyncTest
  ...
  def createSubscriber(probe: WhiteboxSubscriberProbe[Value]): Subscriber[Value] = {
    val underlying = Observer.asSubscriber(new Observer[Value] {
