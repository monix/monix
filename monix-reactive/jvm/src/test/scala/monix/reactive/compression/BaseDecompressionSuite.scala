...
abstract class BaseDecompressionSuite extends BaseOperatorSuite with CompressionTestData {
  ...
  fixture.test("stream of two deflated inputs") { _ =>
  ...
  fixture.test("stream of two deflated inputs as a single chunk") { _ =>
    (jdkCompressedStream(shortText) ++ jdkCompressedStream(otherShortText))
      .bufferTumbling(2)
  ...
  fixture.test("long input, buffer smaller than chunks") { _ =>
  ...
  fixture.test("long input, chunks smaller then buffer") { _ =>
  ...
  fixture.test("fail early if header is corrupted") { _ =>
    Observable
  ...
  fixture.test("fail if input stream finished unexpected") { _ =>
    jdkCompressedStream(longText)
      .map(_.take(20))
      .transform(decompress())
      .toListL
