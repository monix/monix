...
class FlatScanConcurrencySuite extends BaseConcurrencySuite {
  ...
  testService("flatScan should work for synchronous children") { implicit s =>
    val count = 10000L
  ...
  testService("flatScan should work for asynchronous children") { implicit s =>
    val count = 10000L
    val expected = 3L * count * (count - 1) / 2
  ...
  testService(s"flatScan should be cancellable, test 1, count $cancelIterations (issue #468)") { implicit s =>
  ...
  testService(s"flatScan should be cancellable, test 2, count $cancelIterations (issue #468)") { implicit s =>
  ...
  testService(s"flatScan should be cancellable, test 3, count $cancelIterations (issue #468)") { implicit s =>
    for (_ <- 0 until cancelIterations) {
      val p = Promise[Unit]()
      val c = Observable
        .range(0, Long.MaxValue)
