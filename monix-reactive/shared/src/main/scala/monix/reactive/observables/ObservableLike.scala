...
trait ObservableLike[+A, Self[+T] <: ObservableLike[T, Self]] { self: Self[A] =>
  ...
  def doOnComplete(cb: => Unit): Self[A] =
    self.liftByOperator(new DoWorkOnCompleteOperator[A](cb))
  ...
  def onErrorFallbackTo[B >: A](that: => Observable[B]): Self[B] =
    self.onErrorRecoverWith(_ => that)
  ...
  /** Returns an observable that mirrors the behavior of the source,
    ...
    *        throws an error.
    */
  def onErrorRecover[B >: A](f: Throwable => B): Self[B] =
    onErrorRecoverWith { elem => Observable.now(f(elem)) }
  ...
  /** Returns an Observable that mirrors the behavior of the source,
    ...
    *        throws an error.
    */
  def onErrorRecoverWith[B >: A](f: Throwable => Observable[B]): Self[B] =
  ...
  def timeoutOnSlowUpstreamTo[B >: A](timeout: FiniteDuration, backup: => Observable[B]): Self[B] =
    self.timeoutOnSlowUpstream(timeout).onErrorRecoverWith {
