...
/** Defines the available operations for observable-like instances.
  ...
  *         (e.g. all batch emitted by the first observable in the sequence
  ...
  *
  *         You can combine the batch emitted by multiple observables
  ...
  *
  * @define concatReturn an observable that emits batch that are the result of
  *         flattening the batch emitted by the observables emitted by the source
  *
  * @define switchDescription Convert an observable that emits observables
  *         into a single observable that emits the batch emitted by
  *         the most-recently-emitted of those observables.
  *
  * @define switchMapDescription Returns a new observable that emits the batch
  ...
  *         batch (e.g. all batch emitted by the first observable in
  ...
  *         operator.
  *
  * @define mergeReturn an Observable that emits batch that are the
  *         result of flattening the batch emitted by the Observables
  *         emitted by `this`.
  *
  ...
  *
  * @define bufferWithSelectorDesc Periodically gather batch emitted by
  *         an observable into bundles and emit these bundles rather than
  *         emitting the batch one at a time, whenever the `selector`
...
trait ObservableLike[+A, Self[+T] <: ObservableLike[T, Self]]
  ...
  /** Given the source observable and another `Observable`, emits all of
    * the cursor from the first of these Observables to emit an item
  ...
  /** Periodically gather batch emitted by an observable into bundles
    * and emit these bundles rather than emitting the batch one at a
    * time. This version of `buffer` is emitting batch once the
  ...
  /** Returns an observable that emits buffers of batch it collects from
    * the source observable. The resulting observable emits buffers
    * every `skip` batch, each containing `count` batch.
    *
    * If the source observable completes, then the current buffer gets
    * signaled downstream. If the source triggers an error then the
    * current buffer is being dropped and the error gets propagated
    ...
    *
    *  1. in case `skip == count`, then there are no batch dropped and
    ...
    * @param count the maximum size of each buffer before it should
    *        be emitted
    * @param skip how many batch emitted by the source observable should
  ...
  /** Periodically gather batch emitted by an observable into bundles
    ...
    * time.
    *
    * This version of `buffer` emits a new bundle of batch
    * periodically, every timespan amount of time, containing all
    * batch emitted by the source Observable since the previous bundle
    * emission.
    *
    * If the source observable completes, then the current buffer gets
    * signaled downstream. If the source triggers an error then the
  ...
  /** Applies the given partial function to the source
    ...
    * @return an observable that emits the transformed cursor by the
  ...
  /** Creates a new observable from the source and another given
    ...
    *
    * See [[zip]] for an alternative that pairs the cursor in strict sequence.
    *
  ...
  /** Creates a new observable from the source and another given
    ...
    * of the unpaired events are ignored.
    *
    * See [[zipMap]] for an alternative that pairs the cursor
    * in strict sequence.
    *
    * @param other is an observable that gets paired with the source
  ...
  /** Ignores all cursor emitted by the source Observable and only calls
  ...
  /** Only emit an item from an observable if a particular timespan has
    ...
    *
    * Note: If the source observable keeps emitting batch more
    * frequently than the length of the time window, then no batch will
    ...
    *        which that observable emits no batch in order for the
  ...
  /** Doesn't emit anything until a `timeout` period passes without the
    ...
    *
    * Note: If the source observable keeps emitting batch more
    * frequently than the length of the time window, then no batch
  ...
  /** Emit batch from the source, or emit a default item if
    * the source completes after emitting no batch.
    */
  ...
  /** Returns an Observable that emits the cursor emitted by the source
  ...
  /** Given a function that returns a key for each element emitted by
    * the source Observable, suppress duplicates cursor.
    *
    * WARNING: this requires unbounded buffering.
    */
  ...
  /** Suppress duplicate consecutive cursor emitted by the source
    * Observable
    */
  ...
  /** Discard cursor emitted by the source until a second
  ...
  /** Mirror the source observable as long as the source keeps emitting
    * batch, otherwise if `timeout` passes without the source emitting
  ...
  /** Only emits those batch for which the given predicate holds.
    *
    * @param p a function that evaluates the batch emitted by the source
    *        returning `true` if they pass the filter
    * @return a new observable that emits only those batch in the source
  ...
  /** Groups the batch emitted by an Observable according to a specified
    * criterion, and emits these grouped batch as GroupedObservables,
    * one GroupedObservable per group.
    *
    * Note: A [[monix.reactive.observables.GroupedObservable GroupedObservable]]
    * will cache the batch it is to emit until such time as it is
  ...
  /** Alias for [[completed]]. Ignores all cursor emitted by
  ...
  /** Creates a new observable from this observable and another given
    * observable by interleaving their batch into a strictly
    ...
    * `onCompletes`, the batch will then be directly coming from the
    ...
    * batch in strict alternating sequence.
    *
    ...
    *         the batch from both child streams
    */
  ...
  /** Returns an observable that emits the results of invoking a specified
    * selector on cursor emitted by a [[ConnectableObservable]], which shares a single
  ...
  /** Repeats the batch emitted by the source continuously. It
    * caches the generated batch until `onComplete` and repeats them
    * forever.
    *
  ...
  /** Emit the most recent batch emitted by the source within
    ...
    * batch since the last time it was sampled, the observable that
  ...
  /** Returns an observable that, when the specified sampler
    ...
    * to see what item it has most recently emitted since the previous
    * sampling. Note that if the source observable has emitted no
    * cursor since the last time it was sampled, the observable that
  ...
  /** Emit the most recent cursor emitted by an observable within
  ...
  /** Emit the most recent cursor emitted by the source within
    * periodic time intervals.
    *
    * Alias for [[sample]].
    *
  ...
  /** Returns an observable that mirrors the source but applies a timeout
    ...
    *
    * @param timeout maximum duration between emitted cursor before
  ...
  /** Combines the elements emitted by the source with the latest element
    ...
    *
    * Similar with `combineLatest`, but only emits cursor when the single source
  ...
  /** Creates a new observable from this observable and another given
    * observable by combining their batch in pairs in a strict sequence.
    *
    * So the first item emitted by the new observable will be the tuple of the
    * first batch emitted by each of the source observables; the second item
    * emitted by the new observable will be a tuple with the second batch
    ...
    *
    * @param other is an observable that gets paired with the source
    * @return a new observable sequence that emits the paired batch
