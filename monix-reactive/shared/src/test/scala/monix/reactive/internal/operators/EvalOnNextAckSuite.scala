...
object EvalOnNextAckSuite extends TestSuite[TestScheduler] {
  ...
  test("should work for Observable.range asynchronously") { implicit s =>
    ...
    Observable.range(0, 20).doOnNextAckEval((x,_) => Task(sum += x))
  ...
  test("should work for Observable.range synchronously") { implicit s =>
    ...
    Observable.range(0, 20).doOnNextAckEval((x,_) => Task.now(sum += x))
      .unsafeSubscribeFn(new Subscriber[Long] {
        val scheduler = s
        def onError(ex: Throwable): Unit = ()
        def onNext(elem: Long) = Continue
  ...
  test("should work for Observable.now asynchronously") { implicit s =>
    ...
    Observable.now(10L).doOnNextAckEval((x,_) => Task(sum += x))
      .unsafeSubscribeFn(new Subscriber[Long] {
        val scheduler = s
        def onError(ex: Throwable): Unit = ()
        def onNext(elem: Long) = Continue
  ...
  test("should work for Observable.now synchronously") { implicit s =>
    ...
    Observable.now(10L).doOnNextAckEval((x,_) => Task.now(sum += x))
      .unsafeSubscribeFn(new Subscriber[Long] {
        val scheduler = s
        def onError(ex: Throwable): Unit = ()
        def onNext(elem: Long) = Continue
  ...
  test("should protect against user code for Observable.now") { implicit s =>
    ...
    Observable.now(10L).doOnNextAckEval((x,ack) => throw dummy)
      .unsafeSubscribeFn(new Subscriber[Long] {
        val scheduler = s
  ...
  test("should protect against user code for Observable.range") { implicit s =>
    ...
    Observable.range(1,10).doOnNextAckEval((x,ack) => throw dummy)
      .unsafeSubscribeFn(new Subscriber[Long] {
        val scheduler = s
        def onError(ex: Throwable): Unit = errorThrown = ex
        def onNext(elem: Long) = { received += elem; Continue }
