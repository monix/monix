...
object Pad2Suite extends BaseOperatorSuite {
  def createObservable(sourceCount: Int) = Some {
    val o1 = Observable.range(0, sourceCount)
    val o2 = Observable.range(0, sourceCount + 100)

    val o = new monix.reactive.internal.builders.Pad2Observable(o1, o2, Coeval(0L), Coeval(0L))(_ + _)
    Sample(o, count(sourceCount) + 100, sum(sourceCount) + (2 * sourceCount + 99) * 50, Zero, Zero)
  }
  ...
  def observableInError(sourceCount: Int, ex: Throwable) = Some {
    ...
    val o2 = createObservableEndingInError(Observable.range(0, sourceCount + 100), ex)
  ...
  def brokenUserCodeObservable(sourceCount: Int, ex: Throwable) = Some {
    ...
    val o = new monix.reactive.internal.builders.Pad2Observable(o1, o2, Coeval(0L), Coeval(0L))({ (x1, x2) ⇒
  ...
  override def cancelableObservables(): Seq[Sample] = {
      val sample1 = {
        ...
        val o2 = Observable.range(0, 10 + 10).delayOnNext(1.second)
        new monix.reactive.internal.builders.Pad2Observable(o1, o2, Coeval(0L), Coeval(0L))(_+_)
      }
  ...
  test("self starts before other and finishes before other") { implicit s =>
    ...
    Observable.pad2(obs1, obs2, Coeval(0), Coeval(0)).unsafeSubscribeFn(new Observer[(Int, Int)] {
  ...
  test("self signals error and interrupts the stream before it starts") { implicit s =>
    ...
    Observable.pad2(obs1, obs2.doOnDownstreamStop { wasCanceled = true }, Coeval(0), Coeval(0))
  ...
  test("other signals error and interrupts the stream before it starts") { implicit s =>
    ...
    Observable.pad2(obs2.doOnDownstreamStop { wasCanceled = true }, obs1, Coeval(0), Coeval(0))
      .unsafeSubscribeFn(new Observer[(Int, Int)] {
      def onNext(elem: (Int, Int)) = { received = elem; Continue }
      def onError(ex: Throwable) = wasThrown = ex
      def onComplete() = ()
