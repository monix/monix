...
class BufferWithSelectorSuite extends BaseOperatorSuite {
  ...
  fixture.test("selector onComplete should complete the resulting observable") { implicit s =>
    ...
    s.tick(2.seconds)
    assertEquals(f.value, Some(Success(Some(10L * 19L))))
  }

  fixture.test("selector onError should terminate the resulting observable") { implicit s =>
  ...
  fixture.test("sizeOf should weight elements with observable.bufferTimedWithPressure") { implicit s =>
    ...
    s.tick(1.seconds)
    assertEquals(f.value, Some(Success(Some(List(0L, 1L, 2L)))))
  }
}
