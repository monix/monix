...
object OnErrorRetryWithBackoffSuite extends BaseOperatorSuite {
  def createObservable(sourceCount: Int) = Some {
    val ex = DummyException("expected")
    val o = Observable
      .range(0, sourceCount)
      .endWithError(ex)
      .onErrorRetryWithBackoff(3, 2.seconds)
  ...
  def observableInError(sourceCount: Int, ex: Throwable) =
    if (sourceCount <= 1) None
      ...
      Some {
        val o = Observable.range(0, sourceCount).endWithError(ex).onErrorRetryWithBackoff(3, 2.seconds)
