...
class ScanMapSuite extends BaseTestSuite {

  fixture.test("Observable.scanMap equivalence to Observable.scan") { implicit s =>
  ...
  fixture.test("Observable.scanMap0.drop(1) <-> Observable.scanMap") { implicit s =>
    check1 { (obs: Observable[Int]) =>
      obs.scanMap0(x => x).drop(1) <-> obs.scanMap(x => x)
    }
  }

  fixture.test("Observable.scanMap0 emits empty element as head") { implicit s =>
    check1 { (obs: Observable[Int]) =>
