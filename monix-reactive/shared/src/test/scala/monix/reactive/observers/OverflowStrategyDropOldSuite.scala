...
class OverflowStrategyDropOldSuite extends monix.reactive.BaseTestSuite {
  ...
  testScheduler("should not lose events, test 1") { implicit s =>
    ...
    val buffer = BufferedSubscriber[Int](Subscriber(underlying, s), DropOld(1000))
  ...
  testScheduler("should drop old events when over capacity") { implicit s =>
    var received = 0
    var wasCompleted = false
  ...
  testScheduler("subscriber STOP after a synchronous onNext") { implicit s =>
    ...
    val buffer = BufferedSubscriber[Int](underlying, DropOld(16))
  ...
  testScheduler("streaming null is not allowed") { implicit s =>
    ...
    val buffer = BufferedSubscriber[String](underlying, DropOld(16))
    buffer.onNext(null)
