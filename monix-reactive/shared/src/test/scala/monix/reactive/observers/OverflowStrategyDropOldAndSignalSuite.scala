...
class OverflowStrategyDropOldAndSignalSuite extends BaseTestSuite {
  ...
  fixture.test("should drop old events when over capacity and log once") { implicit s =>
    var received = 0
    var wasCompleted = false
    val promise = Promise[Ack]()
