...
class ContramapSubscriberSuite extends BaseTestSuite {
  fixture.test("Subscriber.contramap equivalence with plain Subscriber") { implicit s =>
    check1 { (xs: List[Int]) =>
      var sum = 0
  ...
  fixture.test("Subscriber.contramap protects against user code") { implicit s =>
  ...
  fixture.test("Subscriber.contramap works") { implicit s =>
    var isDone = 0
