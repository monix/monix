...
final class MVar[F[_], A] private (underlying: MVar.AsyncImpl[F, A]) {
...
object MVar {
  ...
  /**
    ...
    */
  def of[F[_], A](initial: A, ps: PaddingStrategy = NoPadding)(implicit F: Async[F]): F[MVar[F, A]] = {
    F.delay(new MVar(new AsyncImpl(Some(initial), ps)))
  }

  /**
    * Builds an empty [[MVar]] instance.
    */
  def empty[F[_], A](ps: PaddingStrategy = NoPadding)(implicit F: Async[F]): F[MVar[F, A]] = {
    F.delay(new MVar(new AsyncImpl(None, ps)))
  }
  ...
  private final class AsyncImpl[F[_], A](initial: Option[A], ps: PaddingStrategy)(
    ...
    extends GenericVar[A, F[Unit]](initial, ps) {
    ...
    def swap(newValue: A): F[A] =
      F.uncancelable { poll =>
        F.flatMap(poll(take))(F.as(put(newValue), _))
      }
    ...
    def modify[B](f: A => F[(A, B)]): F[B] =
      F.uncancelable { poll =>
        F.flatMap(poll(take)) { oldA =>
          F.flatMap(poll(F.onCancel(f(oldA), put(oldA)))) { case (a, b) =>
            F.as(put(a), b)
          }
        }
      }
