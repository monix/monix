...
abstract class IterantBuilders[F[_], Self[+A] <: Iterant[F,A]] extends SharedDocs {
  ...
  /** Alias for [[now]]. */
  final def pure[A](a: A)(implicit F: Applicative[F]): Self[A] = now(a)
  ...
  /** Alias for [[suspend[A](fa* suspend]].
    ...
    * @param fa $suspendByNameParam
    */
  final def defer[A](fa: => Self[A])(implicit F: Applicative[F]): Self[A] = suspend(fa)
...
private[interact] trait IterantBuildersInstances {
