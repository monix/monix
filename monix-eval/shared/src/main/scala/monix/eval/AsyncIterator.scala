...
import monix.eval.AsyncIterator._
import monix.execution.{CancelableFuture, Scheduler}
import scala.collection.{LinearSeq, immutable, mutable}
import scala.util.control.NonFatal

/** Interface producing [[AsyncIterator async iterators]]. */
trait AsyncIterable[+A] extends Serializable {
  /** Creates a [[Task]] that upon evaluation will start
    * an [[AsyncIterator]].
    *
...
object AsyncIterable {
  /** Converts any `scala.collection.Iterable` into
    * an async iterable.
    */
  def fromIterable[A](iterable: scala.collection.Iterable[A]): AsyncIterable[A] =
    new AsyncIterable[A] {
      def asyncIterator(batchSize: Int): Task[AsyncIterator[A]] =
        AsyncIterator.fromIterable(iterable, batchSize)
    }

  /** Converts any `java.lang.Iterable` into
    * an async iterable.
    */
  def fromIterable[A](iterable: java.lang.Iterable[A]): AsyncIterable[A] =
    new AsyncIterable[A] {
      def asyncIterator(batchSize: Int): Task[AsyncIterator[A]] =
        AsyncIterator.fromIterable(iterable, batchSize)
    }

  /** Converts a `List` into an `AsyncIterable`. */
  def fromList[A](list: List[A]): AsyncIterable[A] =
    new AsyncIterable[A] {
      def asyncIterator(batchSize: Int): Task[AsyncIterator[A]] =
        AsyncIterator.fromList(list, batchSize)
    }
}
...
/** An `AsyncIterator` represents a [[Task]] based asynchronous
  ...
  */
sealed trait AsyncIterator[+A] extends Product with Serializable {
  /** Filters the `AsyncIterator` by the given predicate function,
  ...
  /** Alias for [[flatMap]]. */
  final def concatMap[B](f: A => AsyncIterator[B]): AsyncIterator[B] =
    flatMap(f)

  /** Alias for [[concat]]. */
  final def concat[B](implicit ev: A <:< AsyncIterator[B]): AsyncIterator[B] =
    flatten
  ...
  private final def concatTask[B >: A](rhs: Task[AsyncIterator[B]]): AsyncIterator[B] = {
    this match {
      case Wait(task) =>
  ...
  /** Left associative fold using the function 'f'.
    ...
    */
  def foldLeftL[S](seed: S)(f: (S,A) => S): Task[S] =
    this match {
  ...
  /** Left associative fold with the ability to short-circuit the process.
    ...
    */
  def foldWhileL[S](seed: S)(f: (S, A) => (Boolean, S)): Task[S] =
    this match {
      case Empty => Task.now(seed)
      case Error(ex) => Task.error(ex)
      case Wait(next) =>
  ...
  /** Right associative lazy fold on `AsyncIterator` using the
    * folding function 'f'.
    *
    ...
    */
  def foldRightL[B](lb: Task[B])(f: (A, Task[B]) => Task[B]): Task[B] =
    this match {
      case Empty => lb
      case Error(ex) => Task.error(ex)
      case Wait(next) =>
  ...
  /** Returns a new sequence that will take elements from
    ...
    */
  def takeWhile(p: A => Boolean): AsyncIterator[A] =
    this match {
  ...
  /** Recovers from potential errors by mapping them to other
    ...
    */
  def onErrorHandleWith[B >: A](f: Throwable => AsyncIterator[B]): AsyncIterator[B] =
    this match {
      case Empty => Empty
  ...
  /** Recovers from potential errors by mapping them to other
    * async iterators using the provided function.
    */
  def onErrorRecoverWith[B >: A](pf: PartialFunction[Throwable, AsyncIterator[B]]): AsyncIterator[B] =
    onErrorHandleWith {
      case ex if pf.isDefinedAt(ex) => pf(ex)
      case other => AsyncIterator.error(other)
    }

  /** Recovers from potential errors by mapping them to
    * a final element using the provided function.
    */
  def onErrorHandle[B >: A](f: Throwable => B): AsyncIterator[B] =
    onErrorHandleWith(ex => AsyncIterator.now(f(ex)))

  /** Recovers from potential errors by mapping them to
    * a final element using the provided function.
    */
  def onErrorRecover[B >: A](pf: PartialFunction[Throwable, B]): AsyncIterator[B] =
    onErrorHandleWith {
      case ex if pf.isDefinedAt(ex) => AsyncIterator.now(pf(ex))
      case other => AsyncIterator.error(other)
    }

  /** Drops the first `n` elements, from left to right. */
  def drop(n: Int): AsyncIterator[A] =
...
object AsyncIterator {
  ...
  /** Lifts a strict value into an `AsyncIterator` and
  ...
  /** Generages a range between `from` (inclusive) and `until` (exclusive),
    ...
    */
  def range(from: Long, until: Long, step: Long = 1L): AsyncIterator[Long] = {
    def loop(cursor: Long): AsyncIterator[Long] = {
  ...
  /** Converts any sequence into an async iterable.
    ...
    * a strict state.
    */
  def fromList[A](list: immutable.LinearSeq[A], batchSize: Int): Task[AsyncIterator[A]] =
    if (list.isEmpty) Task.now(Empty) else Task.now {
      val (first, rest) = list.splitAt(batchSize)
      NextSeq(first, Task.defer(fromList(rest, batchSize)))
    }

  /** Converts an iterable into an async iterator. */
  def fromIterable[A](iterable: Iterable[A], batchSize: Int): Task[AsyncIterator[A]] =
    Task.now(iterable).flatMap { iter => fromIterator(iter.iterator, batchSize) }

  /** Converts an iterable into an async iterator. */
  def fromIterable[A](iterable: java.lang.Iterable[A], batchSize: Int): Task[AsyncIterator[A]] =
    Task.now(iterable).flatMap { iter => fromIterator(iter.iterator, batchSize) }

  /** Converts a `scala.collection.Iterator` into an async iterator. */
  def fromIterator[A](iterator: scala.collection.Iterator[A], batchSize: Int): Task[AsyncIterator[A]] =
    Task.evalOnce {
      try {
  ...
  def fromIterator[A](iterator: java.util.Iterator[A], batchSize: Int): Task[AsyncIterator[A]] =
    Task.evalOnce {
      try {
        val buffer = mutable.ListBuffer.empty[A]
        var processed = 0
  ...
  /** A state of the [[AsyncIterator]] representing a head/tail decomposition.
    ...
    * @param rest is the next state in the sequence
    */
  final case class NextSeq[+A](headSeq: LinearSeq[A], rest: Task[AsyncIterator[A]]) extends AsyncIterator[A]
  ...
  /** Represents an error state in the iterator.
    ...
    */
  final case class Error(ex: Throwable) extends AsyncIterator[Nothing]
  ...
  // Reusable instances
  private[eval] final val EmptyTask = Task.now(Empty)
}