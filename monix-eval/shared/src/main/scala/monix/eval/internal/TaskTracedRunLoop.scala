...
/**
  * A copy of TaskRunLoop but with Local propagation added
  */
private[eval] object TaskTracedRunLoop {
  ...
  def startMemoization[A](self: MemoizeSuspend[A], context: Context, cb: Callback[A], bindCurrent: Bind, bindRest: CallStack, nextFrame: FrameIndex, local: Local.Context): Boolean = {
