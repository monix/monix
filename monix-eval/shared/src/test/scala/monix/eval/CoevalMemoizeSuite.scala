...
class CoevalMemoizeSuite extends BaseTestSuite {
  fixture.test("Coeval.eval.memoize should work for first subscriber") { _ =>
    var effect = 0
  ...
  fixture.test("Coeval.eval.memoize should work for next subscribers") { _ =>
    var effect = 0
    val coeval = Coeval.eval { effect += 1; effect }.memoize
    coeval.runTry()
  ...
  fixture.test("Coeval.evalOnce.memoize should work for first subscriber") { _ =>
    var effect = 0
  ...
  fixture.test("Coeval.evalOnce.memoize should work for next subscribers") { _ =>
    var effect = 0
    val coeval = Coeval.evalOnce { effect += 1; effect }.memoize
    coeval.runTry()
  ...
  fixture.test("Coeval.now.memoize should return self") { _ =>
    assertEquals(Coeval.now(10), Coeval.now(10).memoize)
  }

  fixture.test("Coeval.error.memoize should return self") { _ =>
    val dummy = DummyException("dummy")
    assertEquals(Coeval.raiseError(dummy), Coeval.raiseError(dummy).memoize)
  }

  fixture.test("Coeval.memoize should be stack safe") { _ =>
    var effect = 0
  ...
  fixture.test("Coeval.apply.memoize effects") { _ =>
    var effect = 0
  ...
  fixture.test("Coeval.suspend.memoize effects") { _ =>
    var effect = 0
  ...
  fixture.test("Coeval.suspend.flatMap.memoize effects") { _ =>
    var effect = 0
    val coeval1 = Coeval
