...
class TaskEvalOnceSuite extends BaseTestSuite {
  fixture.test("Task.evalOnce should work synchronously") { implicit s =>
  ...
  fixture.test("Task.evalOnce should protect against user code errors") { implicit s =>
  ...
  fixture.test("Task.evalOnce.flatMap should be equivalent with Task.evalOnce") { implicit s =>
    val ex = DummyException("dummy")
    val t = Task.evalOnce[Int](if (1 == 1) throw ex else 1).flatMap(Task.now)
    check(t <-> Task.raiseError(ex))
  }

  fixture.test("Task.evalOnce.flatMap should protect against user code") { implicit s =>
    val ex = DummyException("dummy")
    val t = Task.evalOnce(1).flatMap[Int](_ => throw ex)
    check(t <-> Task.raiseError(ex))
  }

  fixture.test("Task.evalOnce.map should work") { implicit s =>
    check1 { (a: Int) =>
      Task.evalOnce(a).map(_ + 1) <-> Task.evalOnce(a + 1)
    }
  }

  fixture.test("Task.evalOnce.flatMap should be tail recursive") { implicit s =>
  ...
  fixture.test("Task.evalOnce should not be cancelable") { implicit s =>
  ...
  fixture.test("Task.evalOnce.coeval") { implicit s =>
    val result = Task.evalOnce(100).runSyncStep
    assertEquals(result, Right(100))
  }

  fixture.test("Task.EvalOnce.runAsync override") { implicit s =>
  ...
  fixture.test("Task.evalOnce.materialize should work for success") { implicit s =>
    val task = Task.evalOnce(1).materialize
    val f = task.runToFuture
    assertEquals(f.value, Some(Success(Success(1))))
  }

  fixture.test("Task.evalOnce.materialize should work for failure") { implicit s =>
    val dummy = DummyException("dummy")
