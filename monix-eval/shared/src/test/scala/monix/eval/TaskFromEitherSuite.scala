...
class TaskFromEitherSuite extends BaseTestSuite {
  fixture.test("Task.fromEither (`E <: Throwable` version) should returns a Now with a Right") { _ =>
    val t = Task.fromEither(Right(10))
    assert(t.isInstanceOf[Now[_]])
  }

  fixture.test("Task.fromEither (`E <: Throwable` version) should succeed with a Right") { implicit s =>
    val t = Task.fromEither(Right(10))
    val f = t.runToFuture
    assertEquals(f.value, Some(Success(10)))
  }

  fixture.test("Task.fromEither (`E <: Throwable` version) should returns an Error with a Left") { _ =>
    val dummy = DummyException("dummy")
    val t = Task.fromEither(Left(dummy))
    assert(t.isInstanceOf[Error[_]])
  }

  fixture.test("Task.fromEither (`E <: Throwable` version) should fail with a Left") { implicit s =>
    val dummy = DummyException("dummy")
    val t = Task.fromEither(Left(dummy))
    val f = t.runToFuture
  ...
  fixture.test("Task.fromEither (`E <: Throwable` version) with a Right should be stack safe") { implicit s =>
  ...
  fixture.test("Task.fromEither (`E <: Throwable` version) with a Left should be stack safe") { implicit s =>
    val count = if (Platform.isJVM) 100000 else 5000
  ...
  fixture.test("Task.fromEither (free `E` version) should returns a Now with a Right") { _ =>
    val t = Task.fromEither(DummyException(_))(Right(10))
    assert(t.isInstanceOf[Now[_]])
  }

  fixture.test("Task.fromEither (free `E` version) should succeed with a Right") { implicit s =>
    val t = Task.fromEither(DummyException(_))(Right(10))
    val f = t.runToFuture
    assertEquals(f.value, Some(Success(10)))
  }

  fixture.test("Task.fromEither (free `E` version) should returns an Error with a Left") { _ =>
    val t = Task.fromEither(DummyException(_))(Left("dummy"))
    assert(t.isInstanceOf[Error[_]])
  }

  fixture.test("Task.fromEither (free `E` version) should fail with a Left") { implicit s =>
    val t = Task.fromEither(DummyException(_))(Left("dummy"))
    val f = t.runToFuture
    assertEquals(f.value, Some(Failure(DummyException("dummy"))))
  }

  fixture.test("Task.fromEither (free `E` version) with a Right should be stack safe") { implicit s =>
    val count = if (Platform.isJVM) 100000 else 5000
  ...
  fixture.test("Task.fromEither (free `E` version) with a Left should be stack safe") { implicit s =>
    val count = if (Platform.isJVM) 100000 else 5000
