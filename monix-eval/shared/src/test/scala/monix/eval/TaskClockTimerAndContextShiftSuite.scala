...
class TaskClockTimerAndContextShiftSuite extends BaseTestSuite {
  fixture.test("Task.clock is implicit") { _ =>
    assertEquals(Task.clock, implicitly[Clock[Task]])
  }

  fixture.test("Task.clock.monotonic") { implicit s =>
    s.tick(1.seconds)

    val f = Task.clock.monotonic(TimeUnit.SECONDS).runToFuture
    s.tick()

    assertEquals(f.value, Some(Success(1L)))
  }

  fixture.test("Task.clock.realTime") { implicit s =>
    s.tick(1.seconds)
  ...
  fixture.test("Task.clock(s2).monotonic") { implicit s =>
  ...
  fixture.test("Task.clock(s).realTime") { implicit s =>
    val s2 = TestScheduler()
    s2.tick(1.seconds)
  ...
  fixture.test("Task.timer(s)") { implicit s =>
    val s2 = TestScheduler()
  ...
  fixture.test("Task.timer(s).clock") { implicit s =>
    val s2 = TestScheduler()
    s2.tick(1.second)
  ...
  fixture.test("Task.contextShift is implicit") { _ =>
    assertEquals(Task.contextShift, implicitly[ContextShift[Task]])
  }

  fixture.test("Task.contextShift.shift") { implicit s =>
  ...
  fixture.test("Task.contextShift.evalOn(s2)") { implicit s =>
    val s2 = TestScheduler()
  ...
  fixture.test("Task.contextShift.evalOn(s2) failure") { implicit s =>
    val s2 = TestScheduler()
  ...
  fixture.test("Task.contextShift.evalOn(s2) uses s2 for async boundaries") { implicit s =>
    val s2 = TestScheduler()
  ...
  fixture.test("Task.contextShift.evalOn(s2) injects s2 to Task.deferAction") { implicit s =>
    val s2 = TestScheduler()
  ...
  fixture.test("Task.contextShift(s).shift") { implicit s =>
  ...
  fixture.test("Task.contextShift(s).evalOn(s2)") { implicit s =>
    val s2 = TestScheduler()
  ...
  fixture.test("Task.contextShift(s).evalOn(s2) failure") { implicit s =>
    val s2 = TestScheduler()
    val dummy = DummyException("dummy")
  ...
  fixture.test("Task.contextShift(s).evalOn(s2) uses s2 for async boundaries") { implicit s =>
    val s2 = TestScheduler()
  ...
  fixture.test("Task.contextShift(s).evalOn(s2) injects s2 to Task.deferAction") { implicit s =>
    val s2 = TestScheduler()
    var wasScheduled = false
    val runnable: Runnable = () => wasScheduled = true
