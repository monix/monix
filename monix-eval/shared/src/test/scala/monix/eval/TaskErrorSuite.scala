...
class TaskErrorSuite extends BaseTestSuite {
  fixture.test("Task.attempt should expose error") { implicit s =>
    val dummy = DummyException("dummy")
    val r = Task.raiseError[Int](dummy).attempt.runSyncStep
    assertEquals(r, Right(Left(dummy)))
  }

  fixture.test("Task.attempt should work for successful values") { implicit s =>
  ...
  fixture.test("Task.eval.materialize") { implicit s =>
    assertEquals(Task.eval(10).materialize.runSyncStep, Right(Success(10)))
  }

  fixture.test("Task.defer.materialize") { implicit s =>
    assertEquals(Task.defer(Task.now(10)).materialize.runSyncStep, Right(Success(10)))
  }

  fixture.test("Task.defer.flatMap.materialize") { implicit s =>
  ...
  fixture.test("Task.flatMap.materialize") { implicit s =>
    assertEquals(Task.eval(10).flatMap(x => Task.now(x)).materialize.runSyncStep, Right(Success(10)))
  }

  fixture.test("Task.materialize") { implicit s =>
  ...
  fixture.test("Task.evalAsync(error).flatMap.materialize") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  fixture.test("Task#onErrorRecover should recover") { implicit s =>
  ...
  fixture.test("Task#onErrorRecover should protect against user code") { implicit s =>
  ...
  fixture.test("Task#onErrorHandle should mirror source on success") { implicit s =>
  ...
  fixture.test("Task#onErrorHandle should recover") { implicit s =>
    val ex = DummyException("dummy")
  ...
  fixture.test("Task#onErrorHandle should protect against user code") { implicit s =>
    val ex1 = DummyException("one")
    val ex2 = DummyException("two")
  ...
  fixture.test("Task.onErrorFallbackTo should mirror source onSuccess") { implicit s =>
  ...
  fixture.test("Task.onErrorFallbackTo should fallback to backup onError") { implicit s =>
    val ex = DummyException("dummy")
  ...
  fixture.test("Task.onErrorFallbackTo should protect against user code") { implicit s =>
    val ex = DummyException("dummy")
  ...
  fixture.test("Task.onErrorFallbackTo should be cancelable if the ExecutionModel allows it") { implicit s =>
  ...
  fixture.test("Task.onErrorRestart should mirror the source onSuccess") { implicit s =>
    var tries = 0
  ...
  fixture.test("Task.onErrorRestart should retry onError") { implicit s =>
    val ex = DummyException("dummy")
    var tries = 0
  ...
  fixture.test("Task.onErrorRestart should emit onError after max retries") { implicit s =>
    val ex = DummyException("dummy")
    var tries = 0
  ...
  fixture.test("Task.onErrorRestart should be cancelable if ExecutionModel permits") { implicit s =>
  ...
  fixture.test("Task.onErrorRestartIf should mirror the source onSuccess") { implicit s =>
    var tries = 0
  ...
  fixture.test("Task.onErrorRestartIf should retry onError") { implicit s =>
    val ex = DummyException("dummy")
    var tries = 0
    val task = Task
  ...
  fixture.test("Task.onErrorRestartIf should emit onError") { implicit s =>
    val ex = DummyException("dummy")
    var tries = 0
    val task = Task
  ...
  fixture.test("Task.onErrorRestartIf should be cancelable if ExecutionModel permits") { implicit s =>
  ...
  fixture.test("Task#onErrorRecoverWith should mirror source on success") { implicit s =>
  ...
  fixture.test("Task#onErrorRecoverWith should recover") { implicit s =>
    val ex = DummyException("dummy")
  ...
  fixture.test("Task#onErrorRecoverWith should protect against user code") { implicit s =>
    val ex1 = DummyException("one")
    val ex2 = DummyException("two")
  ...
  fixture.test("Task#onErrorRecoverWith has a cancelable fallback") { implicit s =>
    val task = Task[Int](throw DummyException("dummy")).onErrorRecoverWith {
      case _: DummyException =>
        Task.evalAsync(99).delayExecution(1.second)
    }
  ...
  fixture.test("Task#onErrorRecover should emit error if not matches") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  fixture.test("Task#onErrorRecoverWith should emit error if not matches") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  fixture.test("Task.eval.flatMap.onErrorRecoverWith should work") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  fixture.test("Task.flatMap.onErrorRecoverWith should work") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  fixture.test("Task.materialize should work for success") { implicit s =>
  ...
  fixture.test("Task.materialize should work for failure") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  fixture.test("Task.materialize should be stack safe") { implicit s =>
    def loop(n: Int): Task[Int] =
  ...
  fixture.test("Task.eval.materialize should work for success") { implicit s =>
    val task = Task.eval(1).materialize
    val f = task.runToFuture
    assertEquals(f.value, Some(Success(Success(1))))
  }

  fixture.test("Task.eval.materialize should work for failure") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  fixture.test("Task.eval.materialize should be stack safe") { implicit s =>
    def loop(n: Int): Task[Int] =
      if (n <= 0) Task.eval(n)
      else
  ...
  fixture.test("Task.now.materialize should work") { implicit s =>
    val task = Task.now(1).materialize
    val f = task.runToFuture
    assertEquals(f.value, Some(Success(Success(1))))
  }

  fixture.test("Task.error.materialize should work") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  fixture.test("Task.materialize on failing flatMap") { implicit s =>
    val ex = DummyException("dummy")
  ...
  fixture.test("Task.now.materialize should be stack safe") { implicit s =>
    def loop(n: Int): Task[Int] =
      if (n <= 0) Task.now(n)
      else
  ...
  fixture.test("Task.raiseError.dematerialize") { implicit s =>
    val ex = DummyException("dummy")
    val result = Task.raiseError[Int](ex).materialize.dematerialize.runToFuture
    assertEquals(result.value, Some(Failure(ex)))
  }

  fixture.test("Task.now.onErrorRecoverWith should be stack safe") { implicit s =>
  ...
  fixture.test("onErrorRecoverWith should be stack safe for loop with executeAsync boundaries") { implicit s =>
    val ex = DummyException("dummy1")
    val count = if (Platform.isJVM) 50000 else 5000
  ...
  fixture.test("Task.onErrorRestartLoop works for success") { implicit s =>
    val dummy = DummyException("dummy")
    var tries = 0
  ...
  fixture.test("Task.onErrorRestartLoop can rethrow") { implicit s =>
    val dummy = DummyException("dummy")
