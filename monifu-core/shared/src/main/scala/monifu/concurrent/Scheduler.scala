...
trait Scheduler extends ExecutionContext with UncaughtExceptionReporter {
  /**
   * Returns the current value of the running virtual machine's
   * high-resolution time source, in nanoseconds.
   *
   * When wanting to measure how long it took for the execution
   * of a callback to happen, do not use `System.nanoTime` directly,
   * prefer this method instead, because then it can be mocked for
   * testing purposes (see for example
   ...
   * {{{
   *   val task = scheduler.scheduleFixedDelay(5.seconds, 10.seconds , new Runnable {
   *     def run() = println("Hello, world!")
   *   })
   *
   *   // later if you change your mind ...
   ...
   * standard output every 10 seconds with an initial delay of 5 seconds:
   * {{{
   *   val task = scheduler.scheduleFixedDelay(5.seconds, 10.seconds)  {
   *     println("Hello, world!")
   *   }
   *
   *   // later if you change your mind ...
  ...
  /**
   * Schedules a task for repeated execution at a fixed rate as given by
   * `period` rate, with an `initialDelay`.
   *
   * For example the following schedules a message to be printed to
   * standard output approximately every 10 seconds with an initial
   * delay of 5 seconds:
   * {{{
