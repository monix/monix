...
package object internal {
  ...
  private[monix] implicit class FutureAckExtensions(val source: Future[Ack]) extends AnyVal {
    ...
    def onContinue(callback: => Unit)(implicit s: Scheduler): Unit =
      source match {
        case sync if sync.isCompleted =>
          if (sync == Continue || (sync != Cancel && sync.value.get == Continue.AsSuccess))
            try callback catch {
              case NonFatal(ex) =>
                s.reportFailure(ex)
            }
    ...
    def ifCanceledDoCancel(cancelable: Cancelable)(implicit s: Scheduler): Future[Ack] = {
      if (source.isCompleted) {
        ...
        source.onComplete {
          ...
          case result@(Cancel.AsSuccess | Failure(_)) =>
            try {
              cancelable.cancel()
              p.complete(result)
            }
    ...
    def onCancelContinue(p: Promise[Ack])(implicit s: Scheduler): Future[Ack] = {
      source match {
        ...
        case async =>
          async.onComplete {
            case Continue.AsSuccess => // nothing
            case Cancel.AsSuccess => p.success(Continue)
    ...
    def ifCanceledDoCancel(p: Promise[Ack])(implicit s: Scheduler): Future[Ack] = {
      source match {
        ...
        case sync if sync.isCompleted =>
          sync.value.get match {
            case Continue.AsSuccess => // do nothing
            case Cancel.AsSuccess => p.success(Cancel)
            case Failure(ex) => p.failure(ex)
            case other =>
              // branch not necessary, but Scala's compiler emits warnings if missing
              s.reportFailure(new MatchError(other.toString))
