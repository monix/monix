...
import monix.internal.{operators, builders}
...
/** The Observable type that implements the Reactive Pattern.
  ...
  *         error is a [[monix.exceptions.CompositeException
...
trait Observable[+T] { self =>
  ...
  /** Subscribes to the stream.
    ...
    * `onError`. If it does, then the behavior is undefined.
    *
    * @param observer is an [[monix.Observer Observer]] that
