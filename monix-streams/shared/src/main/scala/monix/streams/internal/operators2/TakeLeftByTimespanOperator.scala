...
private[streams] final class TakeLeftByTimespanOperator[A](timespan: FiniteDuration)
  extends Operator[A, A] {

  def apply(out: Subscriber[A]): Subscriber[A] =
    new Subscriber[A] with Runnable {
      implicit val scheduler = out.scheduler

      private[this] var isActive = true
      // triggers completion
      private[this] val task: Cancelable =
        scheduler.scheduleOnce(timespan.length, timespan.unit, this)
