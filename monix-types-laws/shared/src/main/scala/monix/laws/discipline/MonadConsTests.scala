...
trait MonadConsTests[F[_]] extends EvaluableTests[F] with MonadFilterTests[F] {
  ...
  def monadCons[A: Arbitrary: Eq, B: Arbitrary: Eq, C: Arbitrary: Eq](implicit
    ...
    new RuleSet {
      val bases = Nil
      val name = "monadCons"
      val parents = Seq(evaluable[A,B,C], monadFilter[A,B,C])
      val props = Seq(
        ...
        "monadCons fromList is consistent with cons" -> forAll(laws.monadConsFromListIsConsistentWithCons[A] _),
