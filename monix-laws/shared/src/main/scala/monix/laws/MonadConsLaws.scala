...
/** Laws for the [[monix.types.MonadCons MonadCons]] type-class.
  *
  * See [[monix.laws.discipline.MonadConsTests MonadConsTests]] for a
  * test specification powered by
  * [[https://github.com/typelevel/discipline/ Discipline]].
  */
trait MonadConsLaws[F[_]] extends MonadFilterLaws[F] with FoldableFLaws[F] {
  implicit def F: MonadCons[F]

  def monadConsFoldLeftFIdentity[A](list: List[A]): IsEq[F[List[A]]] = {
    val fa = list.foldLeft(F.empty[A])((acc,e) => F.cons(e, acc))
    val folded = F.foldLeftF(fa, List.empty[A])((acc, a) => a :: acc)
    folded <-> F.pure(list)
  }
