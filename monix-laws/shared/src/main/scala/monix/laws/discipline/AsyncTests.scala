...
/** Tests specification for the [[monix.types.Async Async]] type-class,
  * powered by [[https://github.com/typelevel/discipline/ Discipline]].
  *
  * See [[AsyncLaws]] for the defined laws.
  */
trait AsyncTests[F[_]] extends NonstrictTests[F] with RecoverableTests[F,Throwable] {
  ...
  def async[A: Arbitrary: Eq, B: Arbitrary: Eq, C: Arbitrary: Eq](implicit
    ...
    new RuleSet {
      val bases = Nil
      val name = "async"
      val parents = Seq(nonstrict[A,B,C], recoverable[A,B,C])
      def props = Seq.empty
    }
  }
}
