...
import monifu.reactive.{Observer, CompositeException, Observable}
import scala.concurrent.duration._
import scala.util.Random

object MergeDelayErrorOneSuite extends BaseOperatorSuite {
  ...
  def create(sourceCount: Int, ex: Throwable = null) = Some {
    ...
    val o = source.mergeMapDelayError(i =>
      Observable.unit(i).endWithError(SomeException(10)))
  ...
  def sum(sourceCount: Int) = {
    sourceCount * (sourceCount - 1) / 2 + sourceCount * 10
  }
