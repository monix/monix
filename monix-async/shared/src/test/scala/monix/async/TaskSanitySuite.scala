...
//  }
//
//  test("Task.async should work for onSuccess") { implicit s =>
//    var wasTriggered = false
//    def trigger(): String = { wasTriggered = true; "result" }
//
//    val task = {
//      val ref = Task.async[String] { (s, cb) => cb.onSuccess(trigger()); Cancelable.empty }
//      Task.fork(ref)
//    }
//
//    assert(!wasTriggered, "!wasTriggered")
...
//  }
//
//  test("Task.async should work for onError") { implicit s =>
//    val ex = DummyException("dummy")
//    val task = {
//      val ref = Task.async[String] { (s, cb) => cb.onError(ex); Cancelable.empty }
//      Task.fork(ref)
//    }
//
//    val f = task.runAsync
...
//  }
//
//  test("Task.async should protect against user code errors") { implicit s =>
//    val ex = DummyException("dummy")
//    val f = Task.async[Int] { (cb,s) => if (1 == 1) throw ex else Cancelable.empty }.runAsync
//
//    s.tick()
//    assertEquals(f.value, Some(Failure(ex)))
...
//    var wasTriggered = false
//    val source = Task.async[String] { (s, callback) =>
