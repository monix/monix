 ...
 * `https://github.com/typelevel/cats-effect/blob/27bb3fdffe86d299b8966eb352fa1b2fd8125395/core/jvm/src/main/scala/cats/effect/unsafe/LocalQueue.scala`.
 */
...
/** Fixed length, FIFO, single producer, multiple consumer, lock-free, circular
  ...
  * underlying `buffer` of [[java.lang.Runnable]] object references.
  *
...
private final class LocalQueue {
  ...
  /** The array of [[java.lang.Runnable]] object references physically backing
  ...
  /** The head of the queue.
    ...
    * value of the head, pointing to the next [[java.lang.Runnable]] instance
    ...
    * to transfer half of the available [[java.lang.Runnable]] object references
  ...
  def enqueue(
    fiber: Runnable,
    batched: ScalQueue[Array[Runnable]],
    overflow: ScalQueue[Runnable],
    ...
    while (true) {
      ...
      if (head.compareAndSet(hd, newHd)) {
        ...
        val batch = new Array[Runnable](OverflowBatchSize)
        var i = 0
  ...
  /** Enqueues a batch of fibers to the local queue as a single operation without
    ...
    */
  def enqueueBatch(batch: Array[Runnable]): Runnable = {
    // A plain, unsynchronized load of the tail of the local queue.
    val tl = tail
  ...
  /** Steals half of the enqueued fibers from this queue and places them into the
    ...
    */
  def stealInto(dst: LocalQueue): Runnable = {
  ...
  /** Steals all enqueued fibers and transfers them to the provided array.
    ...
    *            transferred
    */
  def drain(dst: Array[Runnable]): Unit = {
    // A plain, unsynchronized load of the tail of the local queue.
    val tl = tail
