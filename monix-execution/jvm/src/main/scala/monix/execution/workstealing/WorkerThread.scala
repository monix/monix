 ...
 * `https://github.com/typelevel/cats-effect/blob/27bb3fdffe86d299b8966eb352fa1b2fd8125395/core/jvm/src/main/scala/cats/effect/unsafe/WorkerThread.scala`.
 */
...
private final class WorkerThread(
  ...
  private[workstealing] val index: Int,
  ...
  private[this] val batched: ScalQueue[Array[Runnable]],
  ...
  private[this] val overflow: ScalQueue[Runnable],
  ...
  /** A mutable reference to a fiber which is used to bypass the local queue when
    ...
    */
  private[this] var cedeBypass: Runnable = null
  ...
  override def blockOn[T](thunk: => T)(implicit permission: CanAwait): T = {
    ...
    if (pool.notifyParked(rnd)) {
      ...
      val drain = new Array[Runnable](LocalQueueCapacity)
      queue.drain(drain)
      ...
      if (blocking) {
        ...
        // not an issue, as the `HelperThread`s are all executing `Runnable`
