 ...
 * `https://github.com/typelevel/cats-effect/blob/27bb3fdffe86d299b8966eb352fa1b2fd8125395/core/jvm/src/main/scala/cats/effect/unsafe/WorkStealingThreadPool.scala`.
 */
...
/** Work-stealing thread pool which manages a pool of [[WorkerThread]]s for the
  * specific purpose of executing [[java.lang.Runnable]] instances with
  ...
  */
private[execution] final class WorkStealingThreadPool(
  ...
  /** The batched queue on which spillover work from other local queues can end
    * up.
    */
  private[this] val batchedQueue: ScalQueue[Array[Runnable]] =
    new ScalQueue(threadCount)

  /** The overflow queue on which fibers coming from outside the pool are
    * enqueued.
    */
  private[this] val overflowQueue: ScalQueue[Runnable] =
    new ScalQueue(threadCount)
  ...
  /** Tries to steal work from other worker threads. This method does a linear
    ...
    */
  private[workstealing] def stealFromOtherWorkerThread(dest: Int, random: ThreadLocalRandom): Runnable = {
  ...
  /** Potentially unparks a worker thread.
    ...
    *               their enqueue operations
    */
  private[workstealing] def notifyParked(random: ThreadLocalRandom): Boolean = {
  ...
  /** Notifies a thread if there are fibers available for stealing in any of the
    ...
    *               their enqueue operations
    */
  private[workstealing] def notifyIfWorkPending(random: ThreadLocalRandom): Unit = {
    var i = 0
    while (i < threadCount) {
  ...
  /** Decides whether a worker thread is allowed to enter the searching state
    ...
    */
  private[workstealing] def transitionWorkerToSearching(): Boolean = {
    val st = state.get()
  ...
  /** Deregisters the current worker thread from the set of searching threads and
    ...
    *               their enqueue operations
    */
  private[workstealing] def transitionWorkerFromSearching(random: ThreadLocalRandom): Unit = {
  ...
  /** Updates the internal state to mark the given worker thread as parked.
    ...
    *         `false` otherwise
    */
  private[workstealing] def transitionWorkerToParkedWhenSearching(): Boolean = {
  ...
  /** Updates the internal state to mark the given worker thread as parked.
    ...
    */
  private[workstealing] def transitionWorkerToParked(): Unit = {
  ...
  /** Executes a fiber on this thread pool.
    ...
    */
  private[this] def executeFiber(fiber: Runnable): Unit = {
  ...
  /** Reschedules the given fiber directly on the local work stealing queue on the same thread,
    ...
    * `WorkerThread`.
    */
  private[this] def rescheduleFiber(fiber: Runnable): Unit = {
    val thread = Thread.currentThread()
    if (thread.isInstanceOf[WorkerThread]) {
  ...
  /** Reschedules the given fiber directly on the local work stealing queue on the same thread.
    ...
    */
  private[this] def scheduleFiber(fiber: Runnable): Unit = {
    val thread = Thread.currentThread()
    if (thread.isInstanceOf[WorkerThread]) {
      thread.asInstanceOf[WorkerThread].schedule(fiber)
    } else {
