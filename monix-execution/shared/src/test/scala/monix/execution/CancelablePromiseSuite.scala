...
class CancelablePromiseSuite extends MUnitFunSuite {
  test("completes in success") {
    ...
    val f2 = p.future
    val p3 = Promise[Int]()
    val _ = p.subscribe { v => p3.complete(v); () }
    ...
    val p4 = Promise[Int]()
    val _ = p.subscribe { v => p4.complete(v); () }
  ...
  test("can unsubscribe callbacks") {
    ...
    val p2 = Promise[Int]()
    val _ = p.subscribe { v => p2.complete(v); () }
  ...
  test("already completed in success") {
    ...
    val p1 = Promise[Int]()
    val _ = p.subscribe { v => p1.complete(v); () }
