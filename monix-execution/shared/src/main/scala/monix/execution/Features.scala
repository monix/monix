...
object Features {
  ...
  /** Encodes a feature flag, stored as an `Int`.
    *
    ...
    * }}}
    *
    * You can wrap an `Int` into a `Flag` via [[flag]].
    */
  type Flag <: Int with FlagTag

  /** Encodes a set of [[Flag]] values.
    *
    * Internally this is still an `Int`, but has its own type for
    * type safety reasons.
    */
  type Flags <: Int with FlagsTag
