...
object BuildFromCompat {
  ...
  implicit def fromCanBuildFrom[From, A, C](implicit cbf: CanBuildFrom[From, A, C]): BuildFromCompat[From, A, C] =
    new BuildFromCompat[From, A, C] {
      def newBuilder(from: From): mutable.Builder[A, C] = cbf.apply(from)
    }
