...
object delayFirst {
   ...
   */
  def apply[T](policy: BufferPolicy, init: (() => Unit, Throwable => Unit) => Cancelable)(implicit s: Scheduler) =
   ...
   */
  def apply[T](policy: BufferPolicy, future: Future[_])(implicit s: Scheduler) =
    (source: Observable[T]) =>
      source.delayFirst(policy, { (connect, signalError) =>
