...
/**
 * Wraps a [[monifu.Observer Observer]] instance into an
 ...
 * contract.
 *
 * Given that when emitting [[monifu.Observer.onNext Observer.onNext]] calls,
...
private[monifu] object SubscriberAsReactiveSubscriber {
   ...
   *                    also representing the buffer size; MUST BE strictly positive
   */
  def apply[T](subscriber: monifu.Subscriber[T], requestCount: Int = 128): Subscriber[T] =
    subscriber match {
 ...
 * contract.
 *
 * Given that we can guarantee a [[monifu.observers.SynchronousObserver SynchronousObserver]]
