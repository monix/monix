...
/**
 * Wraps a [[monix.Observer Observer]] instance into an
 ...
 * contract.
 *
 * Given that when emitting [[monix.Observer.onNext Observer.onNext]] calls,
...
private[monix] final class SubscriberAsReactiveSubscriber[T] private
    (subscriber: monix.Subscriber[T], requestCount: Int)
...
private[monix] object SubscriberAsReactiveSubscriber {
  /**
   * Wraps a [[monix.Observer Observer]] instance into a
   * `org.reactivestreams.Subscriber` instance. The resulting
   * subscriber respects the [[http://www.reactive-streams.org/ Reactive Streams]]
   * contract.
   *
   ...
   *                    also representing the buffer size; MUST BE strictly positive
   */
  def apply[T](subscriber: monix.Subscriber[T], requestCount: Int = 128): Subscriber[T] =
    subscriber match {
...
/**
 * Wraps a [[monix.observers.SynchronousObserver SynchronousObserver]] instance into a
 * `org.reactivestreams.Subscriber` instance. The resulting
 * subscriber respects the [[http://www.reactive-streams.org/ Reactive Streams]]
 * contract.
 *
 * Given that we can guarantee a [[monix.observers.SynchronousObserver SynchronousObserver]]
