...
import monix.concurrent.schedulers.SchedulerCompanion
...
@implicitNotFound(
  ...
  "import monix.concurrent.Implicits.globalScheduler or use a custom one")
trait Scheduler extends ExecutionContext with UncaughtExceptionReporter {
  ...
  /** Returns the current time in milliseconds.  Note that while the
    ...
    * [[monix.concurrent.schedulers.TestScheduler TestScheduler]])
    */
