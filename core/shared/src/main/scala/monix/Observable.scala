...
import monix.observables.{CachedObservable, ConnectableObservable, GroupedObservable}
import monix.observers._
import monix.subjects.{AsyncSubject, BehaviorSubject, PublishSubject, ReplaySubject}
...
/** The Observable interface in the Rx pattern.
  ...
  * [[monix.observers.BufferedSubscriber BufferedSubscriber]].
  *
  ...
  *                                error is a [[monix.exceptions.CompositeException CompositeException]],
  ...
  *                                  downstream [[monix.Observer Observer]] given in
  ...
  *                                  [[monix.OverflowStrategy overflowStrategy]], see
  *                                  [[monix.OverflowStrategy OverflowStrategy]] for
  *                                  options.
  */
trait Observable[+T] { self =>
  ...
  /** Groups the items emitted by an Observable according to a specified
    ...
    *
    * Note: A [[monix.observables.GroupedObservable GroupedObservable]]
  ...
  /** Groups the items emitted by an Observable according to a specified
    ...
    * one GroupedObservable per group.
    *
    * A [[monix.observables.GroupedObservable GroupedObservable]]
    * will cache the items it is to emit until such time as it is
    * subscribed to. For this reason, in order to avoid memory leaks,
    * you should not simply ignore those GroupedObservables that do not
    * concern you. Instead, you can signal to them that they may
  ...
  /** Applies a binary operator to a start value and to elements produced
    ...
    *
    * It's the combination between [[monix.Observable.scan scan]]
    * and [[monix.Observable.flatten]].
    */
  ...
  /** Applies a binary operator to a start value and to elements produced
    ...
    *
    * It's the combination between [[monix.Observable.scan scan]]
    * and [[monix.Observable.flattenDelayError]].
    */
  ...
  /** Caches the emissions from the source Observable and replays them
    ...
    * returning a [[monix.observables.ConnectableObservable ConnectableObservable]]
    * for which you must call
    * [[monix.observables.ConnectableObservable.connect connect]]
  ...
  /** Caches the emissions from the source Observable and replays them
    ...
    * [[monix.observables.ConnectableObservable ConnectableObservable]]
    * for which you must call
    * [[monix.observables.ConnectableObservable.connect connect]]
    * to activate the subscription.
    *
  ...
  /** Converts this observable into a multicast observable, useful for turning a cold observable into
    * a hot one (i.e. whose source is shared by all observers). The underlying subject used is a
    * [[monix.subjects.BehaviorSubject BehaviorSubject]].
    */
  ...
  /** Converts this observable into a multicast observable, useful for turning a cold observable into
    * a hot one (i.e. whose source is shared by all observers). The underlying subject used is a
    * [[monix.subjects.ReplaySubject ReplaySubject]].
    */
  ...
  /** Converts this observable into a multicast observable, useful for turning a cold observable into
    * a hot one (i.e. whose source is shared by all observers). The underlying subject used is a
    * [[monix.subjects.AsyncSubject AsyncSubject]].
    */
