...
final class IterantTakeEveryNthSuite extends BaseTestSuite {
  ...
  testScheduler("Iterant[Task].takeEveryNth equivalence with naiveImp") { implicit s =>
  ...
  testScheduler("Iterant.takeEveryNth protects against broken batches") { implicit s =>
  ...
  testScheduler("Iterant.takeEveryNth protects against broken cursors") { implicit s =>
    check1 { (iter: Iterant[Task, Int]) =>
      val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.takeEveryNth triggers early stop on exception") { _ =>
  ...
  testScheduler("Iterant.takeEveryNth throws on invalid n") { implicit s =>
