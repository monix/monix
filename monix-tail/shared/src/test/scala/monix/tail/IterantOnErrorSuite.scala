...
class IterantOnErrorSuite extends BaseTestSuite {
  fixture.test("fa.attempt <-> fa.map(Right) for successful streams") { implicit s =>
  ...
  fixture.test("fa.attempt ends with a Left in case of error") { implicit s =>
  ...
  fixture.test("fa.attempt.flatMap <-> fa") { implicit s =>
  ...
  test("fa.onErrorHandleWith(_ => fb) <-> fa for successful streams") {
  ...
  fixture.test("fa.onErrorHandleWith(_ => fb) <-> fa ++ fb in case of error") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  fixture.test("Iterant[Task].onErrorHandleWith should protect against broken batches") { implicit s =>
  ...
  fixture.test("Iterant[Task].onErrorHandleWith should protect against cursors broken in the middle") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  test("onErrorHandleWith should protect against broken continuations") {
    ...
    for (broken <- brokenTails) {
      ...
      assertEquals(out.takeRight(fallback.length), fallback.toList)
    }
  }

  test("onErrorHandleWith should stack finalizers") {
    var effect = 0
  ...
  test("attempt should protect against broken continuations") {
    for (broken <- brokenTails) {
  ...
  test("attempt should stack finalizers") {
    var effect = 0
  ...
  test("onErrorIgnore should capture exceptions from eval, mapEval & liftF") {
    val dummy = DummyException("dummy")
  ...
  test("attempt should capture exceptions from mapEval") {
    val dummy = DummyException("dummy")
  ...
  test("attempt should protect against broken batches") {
    val dummy = DummyException("dummy")
    val result =
  ...
  test("attempt should protect against broken cursor") {
    val dummy = DummyException("dummy")
    val result = Iterant[IO]
  ...
  test("attempt should protect against cursors broken in the middle") {
    val dummy = DummyException("dummy")
    val cursor = BatchCursor.fromIterator(semiBrokenIterator(dummy))
  ...
  test("Resource.attempt with broken acquire") {
    val dummy = DummyException("dummy")
  ...
  test("Resource.attempt with broken use") {
    val dummy = DummyException("dummy")
  ...
  test("Resource.attempt with broken release") {
    val dummy = DummyException("dummy")
    val stream = 1 +: Iterant[Coeval].scopeS[Int, Int](
      Coeval(1),
  ...
  test("Resource.attempt with broken use and release") {
  ...
  test("Resource.onErrorHandleWith with broken acquire") {
    val dummy = DummyException("dummy")
    val stream = 1 +: Iterant[Coeval].resource(Coeval.raiseError[Int](dummy))(_ => Coeval.unit)
  ...
  test("Resource.onErrorHandleWith with broken use") {
    val dummy = DummyException("dummy")
    val stream = 1 +: Iterant[Coeval].scopeS[Int, Int](
      Coeval(1),
      _ => Coeval.raiseError[Iterant[Coeval, Int]](dummy),
  ...
  test("Resource.onErrorHandleWith with broken release") {
    val dummy = DummyException("dummy")
    val stream = 1 +: Iterant[Coeval].scopeS[Int, Int](
      Coeval(1),
      i => Coeval(Iterant.pure(i + 1)),
  ...
  test("Resource.onErrorHandleWith with broken use and release") {
    val dummy1 = DummyException("dummy1")
    val dummy2 = DummyException("dummy2")
