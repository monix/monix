...
final class IterantTakeWhileSuite extends BaseTestSuite {
  ...
  testScheduler("Iterant[Task].takeWhile equivalence with List.takeWhile") { implicit s =>
  ...
  testScheduler("Iterant[Task].takeWhile works for non-determinate batches") { implicit s =>
  ...
  testScheduler("Iterant[Task].takeWhile(_ => true) mirrors the source") { implicit s =>
    check1 { (iter: Iterant[Coeval, Int]) =>
      iter <-> iter.takeWhile(_ => true)
    }
  }

  testScheduler("Iterant[Coeval].takeWhile preserves resource safety") { implicit s =>
  ...
  testScheduler("Iterant.takeWhile protects against broken batches") { implicit s =>
  ...
  testScheduler("Iterant.takeWhile protects against broken cursors") { implicit s =>
    check1 { (iter: Iterant[Task, Int]) =>
      val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.takeWhile protects against user code") { implicit s =>
    check1 { (iter: Iterant[Task, Int]) =>
      val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.takeWhile preserves resource safety on exception") { _ =>
    check1 { (iter: Iterant[Coeval, Int]) =>
      val cancelable = BooleanCancelable()
      val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.takeWhile preserves the source guarantee") { implicit s =>
    var effect = 0
