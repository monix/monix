...
final class IterantTakeLastSuite extends BaseTestSuite {
  testScheduler("Iterant.takeLast is equivalent with List.takeRight") { implicit s =>
  ...
  testScheduler("Iterant.takeLast protects against broken batches") { implicit s =>
  ...
  testScheduler("Iterant.takeLast protects against broken cursors") { implicit s =>
    check1 { (iter: Iterant[Task, Int]) =>
      val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.takeLast preserves the source guarantee") { implicit s =>
    var effect = 0
