...
final class IterantFilterSuite extends BaseTestSuite {
  testScheduler("Iterant.filter <=> List.filter") { implicit s =>
  ...
  testScheduler("Iterant.filter protects against user error") { implicit s =>
  ...
  testScheduler("Iterant.filter flatMap equivalence") { implicit s =>
    check2 { (stream: Iterant[Task, Int], p: Int => Boolean) =>
  ...
  testScheduler("Iterant.filter suspends the evaluation for NextBatch") { _ =>
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.filter suspends the evaluation for NextCursor") { _ =>
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.filter protects against user code for Next") { _ =>
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.filter protects against user code for Last") { _ =>
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.filter doesn't touch Halt") { _ =>
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.filter preserves resource safety") { implicit s =>
    var effect = 0
  ...
  testScheduler("Iterant.withFilter applies filtering in for-comprehension") { implicit s =>
    val source = Iterant[Coeval]
