...
final class IterantTakeWhileWithIndexSuite extends BaseTestSuite {
  ...
  testScheduler("naiveImp smoke test") { implicit s =>
  ...
  testScheduler("Iterant[Task].takeWhileWithIndex((_, _) => true) mirrors the source") { implicit s =>
    check1 { (iter: Iterant[Coeval, Int]) =>
      iter <-> iter.takeWhileWithIndex((_, _) => true)
    }
  }

  testScheduler("Iterant[Task].takeWhileWithIndex equivalence with naiveImp") { implicit s =>
  ...
  testScheduler("Iterant[Task].takeWhileWithIndex works for non-determinate batches") { implicit s =>
  ...
  testScheduler("Iterant[Coeval].takeWhileWithIndex preserves resource safety") { implicit s =>
  ...
  testScheduler("Iterant.takeWhileWithIndex protects against broken batches") { implicit s =>
  ...
  testScheduler("Iterant.takeWhileWithIndex protects against broken cursors") { implicit s =>
    check1 { (iter: Iterant[Task, Int]) =>
      val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.takeWhileWithIndex protects against user code") { implicit s =>
    check1 { (iter: Iterant[Task, Int]) =>
      val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.takeWhileWithIndex preserves resoure safety on exception") { _ =>
    check1 { (iter: Iterant[Coeval, Int]) =>
      val cancelable = BooleanCancelable()
      val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.takeWhileWithIndex preserves the source guarantee") { implicit s =>
    var effect = 0
