...
final class IterantResourceSuite extends BaseTestSuite {
  ...
  testScheduler("Iterant.resource.flatMap(use) yields all elements `use` provides") { _ =>
  ...
  testScheduler("Iterant.resource.flatMap(use) preserves earlyStop of stream returned from `use`") { _ =>
  ...
  testScheduler("Iterant.resource releases resource on normal completion") { _ =>
    val rs = new Resource
    val bracketed = Iterant
  ...
  testScheduler("Iterant.resource releases resource on early stop") { _ =>
    val rs = new Resource
    val bracketed = Iterant
      .resource(rs.acquire)(_.release)
      .flatMap(_ => Iterant.range(1, 10))
  ...
  testScheduler("Iterant.resource releases resource on exception") { _ =>
    val rs = new Resource
  ...
  testScheduler("Iterant.resource.flatMap(use) releases resource if `use` throws") { _ =>
    val rs = new Resource
  ...
  testScheduler("Iterant.resource does not call `release` if `acquire` has an error") { _ =>
    val rs = new Resource
    val dummy = DummyException("dummy")
    val bracketed = Iterant
  ...
  testScheduler("resource(r)(_ => raiseError(e)).flatMap(_ => fa) <-> fa ++ raiseError(e)") { _ =>
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.resource nesting: outer releases even if inner release fails") { _ =>
    var released = false
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.resource.flatMap(child) calls release when child is broken") { _ =>
    var released = false
    val dummy = DummyException("dummy")
    val bracketed = Iterant.resource(Coeval.unit)(_ => Coeval { released = true }).flatMap { _ =>
  ...
  testScheduler("Iterant.resource nesting: inner releases even if outer release fails") { _ =>
    var released = false
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.resource handles broken batches & cursors") { _ =>
    val rs = new Resource
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.resource handles broken `next` continuations") { _ =>
    val rs = new Resource
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.resource releases resource on all completion methods") { _ =>
    val rs = new Resource
  ...
  testScheduler("Iterant.resource does not require non-strict use") { _ =>
