...
final class IterantFromListSuite extends BaseTestSuite {
  testScheduler("Iterant[Task].fromList") { implicit s =>
  ...
  testScheduler("Iterant[Task].fromList (async)") { implicit s =>
    check1 { (list: List[Int]) =>
  ...
  testScheduler("Iterant[Coeval].fromList") { implicit s =>
    check1 { (list: List[Int]) =>
  ...
  testScheduler("Iterant[Coeval].fromList (async)") { implicit s =>
    check1 { (list: List[Int]) =>
