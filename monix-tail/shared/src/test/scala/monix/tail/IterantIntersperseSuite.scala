...
class IterantIntersperseSuite extends BaseTestSuite {
  def intersperseList[A](separator: A)(xs: List[A]): List[A] = {
    xs.flatMap(e => e :: separator :: Nil).dropRight(1)
  }

  fixture.test("Iterant.intersperse(x) inserts the separator between elem pairs") { implicit s =>
    check2 { (stream: Iterant[Coeval, Int], elem: Int) =>
      stream.intersperse(elem).toListL <-> stream.toListL.map(intersperseList(elem))
    }
  }

  fixture.test("Iterant.intersperse(x) releases the source's resources") { implicit s =>
  ...
  fixture.test("Iterant.intersperse(x) protects against broken batches") { implicit s =>
    val builders = Iterant[Coeval]
    import builders._
  ...
  fixture.test("Iterant.intersperse(x) protects against broken cursors") { implicit s =>
    val builders = Iterant[Coeval]
    import builders._
  ...
  fixture.test("Iterant.intersperse(a, b, c) inserts the separator between elem pairs and adds prefix/suffix") {
    implicit s =>
