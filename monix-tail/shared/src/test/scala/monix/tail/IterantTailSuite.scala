...
final class IterantTailSuite extends BaseTestSuite {
  testScheduler("Iterant.tail is equivalent with List.tail") { implicit s =>
  ...
  testScheduler("Iterant.tail protects against broken batches") { implicit s =>
  ...
  testScheduler("Iterant.tail protects against broken cursors") { implicit s =>
    check1 { (iter: Iterant[Task, Int]) =>
      val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.tail suspends execution for NextCursor") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  testScheduler("Iterant.tail suspends execution for NextBatch") { implicit s =>
    val dummy = DummyException("dummy")
    val iter = Iterant[Coeval]
      .nextBatchS[Int](
