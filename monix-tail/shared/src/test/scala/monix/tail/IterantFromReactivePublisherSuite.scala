...
final class IterantFromReactivePublisherSuite extends BaseTestSuite {
  ...
  testScheduler("fromReactivePublisher(bufferSize = 1) emits values in correct order") { implicit s =>
    check1 { (range: Range) =>
  ...
  testScheduler("fromReactivePublisher(bufferSize = 1) can end in error") { implicit s =>
    check1 { (range: Range) =>
  ...
  testScheduler("fromReactivePublisher(bufferSize = default) emits values in correct order") { implicit s =>
    check1 { (range: Range) =>
      val publisher = new RangePublisher(range, None)
  ...
  testScheduler("fromReactivePublisher(bufferSize = default) can end in error") { implicit s =>
    check1 { (range: Range) =>
      val dummy = DummyException("dummy")
      val publisher = new RangePublisher(range, Some(dummy))
  ...
  testScheduler("fromReactivePublisher(bufferSize = default) with slow consumer") { implicit s =>
    check1 { (range: Range) =>
      val publisher = new RangePublisher(range, None)
  ...
  testScheduler("fromReactivePublisher cancels subscription on earlyStop") { implicit s =>
  ...
  testScheduler("fromReactivePublisher propagates errors") { implicit s =>
    val dummy = DummyException("dummy")
  ...
  testScheduler("fromReactivePublisher(it.toReactivePublisher) is identity") { implicit s =>
    check1 { (it: Iterant[IO, Int]) =>
      Iterant[IO].fromReactivePublisher(it.toReactivePublisher) <-> it
    }
  }

  testScheduler("fromReactivePublisher handles immediate completion") { implicit s =>
