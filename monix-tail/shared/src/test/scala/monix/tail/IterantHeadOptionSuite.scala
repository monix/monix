...
final class IterantHeadOptionSuite extends BaseTestSuite {
  testScheduler("Iterant.headOptionL <-> List.headOption") { _ =>
  ...
  testScheduler("Iterant.headOption <-> List.headOption") { _ =>
    check2 { (list: List[Int], idx: Int) =>
      val iter = arbitraryListToIterant[Coeval, Int](list, math.abs(idx % 4), allowErrors = false)
  ...
  testScheduler("Iterant.headOption suspends execution for NextCursor or NextBatch") { _ =>
  ...
  testScheduler("Iterant.headOption works for empty NextCursor or NextBatch") { _ =>
  ...
  testScheduler("Iterant.headOption doesn't touch Halt") { implicit s =>
  ...
  testScheduler("Iterant.headOption earlyStop gets called for failing `rest` on Next node") { implicit s =>
    var effect = 0
  ...
  testScheduler("headOptionL handles Scope's release before the rest of the stream") { implicit s =>
  ...
  testScheduler("headOptionL handles Scope's release after use is finished") { implicit s =>
    val triggered = Atomic(false)
    val fail = DummyException("fail")
