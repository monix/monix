...
final class IterantDumpSuite extends BaseTestSuite {
  ...
  testScheduler("Iterant.dump works for Next") { implicit s =>
    check1 { (el: Int) =>
  ...
  testScheduler("Iterant.dump works for NextCursor") { implicit s =>
    check1 { (el: Int) =>
      val counter = AtomicInt(0)
      val out =
  ...
  testScheduler("Iterant.dump works for NextBatch") { implicit s =>
    check1 { (el: Int) =>
      val counter = AtomicInt(0)
  ...
  testScheduler("Iterant.dump works for Suspend") { implicit s =>
    val counter = AtomicInt(0)
  ...
  testScheduler("Iterant.dump works for Last") { implicit s =>
    check1 { (el: Int) =>
      val counter = AtomicInt(0)
  ...
  testScheduler("Iterant.dump works for Halt") { implicit s =>
  ...
  testScheduler("Iterant.dump works for Concat") { implicit s =>
    check1 { (el: Int) =>
      val counter = AtomicInt(0)
  ...
  testScheduler("Iterant.dump works for Resource") { implicit s =>
    check1 { (el: Int) =>
      val counter = AtomicInt(0)
  ...
  testScheduler("Iterant.dump preserves the source guarantee") { implicit s =>
    var effect = 0
  ...
  testScheduler("Iterant.dump protects against broken batches") { implicit s =>
  ...
  testScheduler("Iterant.dump protects against broken cursors") { implicit s =>
    check1 { (iter: Iterant[Task, Int]) =>
      val dummy = DummyException("dummy")
      val prefix = iter.onErrorIgnore
  ...
  testScheduler("Iterant.dump protects against user error") { implicit s =>
