...
object CursorBuildersSuite extends BaseTestSuite {
  test("apply") { _ =>
    check1 { (list: List[Int]) =>
      list == Cursor(list:_*).toList
    }
  }

  test("range(0, 100)") { _ =>
    val range = Cursor.range(0, 100)
    assertEquals(range.toList, (0 until 100).toList)
  }

  test("range(0, 100, 2)") { _ =>
    val range = Cursor.range(0, 100, 2)
    assertEquals(range.toList, 0.until(100, 2).toList)
  }

  test("range(100, 0, -1)") { _ =>
    val range = Cursor.range(100, 0, -1)
