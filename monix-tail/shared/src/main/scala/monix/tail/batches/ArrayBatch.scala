...
/** [[monix.tail.Batch Batch]] implementation that wraps
...
final class ArrayBatch[@specialized(Boolean, Byte, Char, Int, Long, Double) A]
  ...
  override def map[B](f: (A) => B): ArrayBatch[B] = {
    ...
    new ArrayBatch(ref.array.asInstanceOf[Array[AnyRef]], 0, ref.length, batches.arrayAnyRefBuilder)
      .asInstanceOf[ArrayBatch[B]]
  }
