...
object Batch {
  ...
  def fromIterable[A](iter: Iterable[A], recommendedBatchSize: Int): Batch[A] =
    new GenericBatch[A] { def cursor() = BatchCursor.fromIterator(iter.iterator, recommendedBatchSize) }
  ...
  def range(from: Int, until: Int, step: Int = 1): Batch[Int] =
    new GenericBatch[Int] { def cursor() = BatchCursor.range(from, until, step) }
}