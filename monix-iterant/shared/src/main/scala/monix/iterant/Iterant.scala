...
/** The `Iterant` is a type that describes lazy, possibly asynchronous
  ...
  *
  *  - [[monix.iterant.Iterant.Next Next]] which signals a single strict
  *    element, the `head` and a `rest` representing the rest of the stream
  *  - [[monix.iterant.Iterant.NextSeq NextSeq]] is a variation on `Next`
  *    for signaling a whole strict batch of elements as the `cursor`,
  *    along with the `rest` representing the rest of the stream
  *  - [[monix.iterant.Iterant.Suspend Suspend]] is for suspending the
  *    evaluation of a stream
  *  - [[monix.iterant.Iterant.Halt Halt]] represents an empty
  *    stream, signaling the end, either in success or in error
  *  - [[monix.iterant.Iterant.Last Last]] represents a one-element
...
private[iterant] trait StreamInstances extends StreamInstances1 {
...
private[iterant] trait StreamInstances1 extends StreamInstances0 {
...
private[iterant] trait StreamInstances0 {
