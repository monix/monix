...
import monix.iterant.cursors.{ArrayCursor, EmptyCursor, IteratorCursor}
...
/** Similar to Java's and Scala's `Iterator`, the `Cursor` type can
  ...
  * Contract:
  *
  *  - the [[monix.iterant.Cursor#current current]] method does not trigger
  ...
  *    the [[monix.iterant.Cursor#moveNext moveNext()]] method; this
